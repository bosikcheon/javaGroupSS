    ...
    <c:forEach var="vo" items="${vos}" varStatus="st">
      <c:if test="${vo.openSw =='공개' || sMid == vo.mid || sLevel == 0}">
		    <tr>
		      <td>${curScrStartNo}</td>
		      <td class="text-start">
		        <c:if test="${vo.claim == 'NO' || sMid == vo.mid || sLevel == 0}"><a href="boardContent?idx=${vo.idx}&pag=${pageVO.pag}&pageSize=${pageVO.pageSize}" class="text-decoration-none text-dark link-primary">${vo.title}</a></c:if> 
		        <c:if test="${vo.claim != 'NO' && sMid != vo.mid && sLevel != 0}"><a href="javascript:alert('현재글은 신고된 글입니다.')" class="text-decoration-none text-dark link-primary">${vo.title}</a></c:if> 
		        <c:if test="${vo.time_diff <= 24}"><img src="${ctp}/images/new.gif" /></c:if>
		        <c:if test="${vo.replyCnt != 0}"><span title="댓글수">(${vo.replyCnt})</span></c:if>
		      </td>
		      <td>
		        <c:if test="${empty vo.content}"><c:set var="content" value="내용없음" /></c:if>
      			...
      			<a href="#" onclick='contentView("${content}")' data-bs-toggle="modal" data-bs-target="#myModal" class="text-decoration-none text-dark link-primary">${vo.nickName}</a>
		      </td>
